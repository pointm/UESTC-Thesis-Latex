...
此时仿照之前的6-11GHz 频段所设计的脊波导配圆形窗，并且使用圆角矩形结构作为脊波导和圆窗之间的过渡段进行设计。初步进行频段搬移并且进行优化之后，得到的窗片结构如图\ref{fig:L波段脊波导窗的整体结构}所示：
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.5\linewidth]{pic/chapter4/L波段脊波导窗的整体结构.png}
    \caption{L波段脊波导窗的整体结构}
    \label{fig:L波段脊波导窗的整体结构}
\end{figure}

此时窗片的具体几何参数如图\ref{fig:L波段脊波导窗的几何参数}所示，此时的L波段脊波导窗纵向参数如图\ref{fig:L波段脊波导窗的几何参数} (a)所示，此时窗片厚度为5.91mm，圆角矩形过渡段的厚度为3.10mm；圆形窗片的直径如图\ref{fig:L波段脊波导窗的几何参数} (b)，圆形窗片的直径为85.14mm；圆角矩形的参数如图\ref{fig:L波段脊波导窗的几何参数} (c)所示，此时圆角矩形的长边长度为69.2mm，宽边长度为12.63mm，倒圆角的半径为6.41mm。

\begin{figure}[!htb]
    \small
    \centering
    \begin{tabular}{@{\ }c@{\ }c}
        \includegraphics[width=0.25\textwidth]{pic/chapter4/L波段窗纵向长度ORI.png} & 
        \hspace{5pt}
        \includegraphics[width=0.25\textwidth]{pic/chapter4/L波段圆形窗片.png}     \\
        \mbox{\small (a)L波段脊波导圆窗纵向长度}                                                                               & 
        \mbox{\small (b)L波段脊波导圆窗参数}                                                               \\[6bp]
        \multicolumn{2}{c}{\includegraphics[width=0.5\textwidth]{pic/chapter4/L波段圆角矩形过渡段ORI.png}} \\  % 使用跨列居中
        \multicolumn{2}{c}{\mbox{\small (c)L波段脊波导圆角矩形过渡段参数}}             
    \end{tabular}
    \caption{L波段脊波导窗的几何参数}
    \label{fig:L波段脊波导窗的几何参数}
\end{figure}
