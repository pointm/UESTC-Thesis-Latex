...
\subsection{同轴窗的理论研究}
相比于波导窗，由于其基本模式为$TEM$模式的原因，基模的截止频率为0，次高模式截止频率距离其基本模式截止频率更远，带宽更宽。
\section{阻抗匹配的设计}
...
\end{equation}
其中参数可被表示为等式\ref{eq:TRM_DW_parameters}中的\ref{eq:Y01}到\ref{eq:theta2}：
\begin{subequations}\label{eq:TRM_DW_parameters}
    ...
    \end{align}
\end{subequations}
式子\ref{eq:TRM_DW_parameters}中的参数$a$为脊波导长边长，参数$b$为脊波导短边长，$s$为脊的宽度，$d$为脊凸起后留下的空腔的缝隙宽度，$\omega$为当前的角频率，$\mu_0$为真空的磁导率，$k_c$为波导的波数，$\lambda_c$为截止频率的波长。
