...
前面已经使用CST针对器件的S参数进行了仿真并且已经验明原先的器件的S参数符合设计要求，本次将使用CST中的PIC模块对原先的器件进行使用基于二次电子倍增的功率容量仿真。首先根据文献\cite{li_2021_novel_multipactor}能获得关于无氧铜的二次电子发射曲线。从文中可以得到，无氧铜的二次电子发射曲线中的关键值为：$114514$。此时根据文献REFERROR可以得到关于蓝宝石的二次电子发射曲线。本文后续将使用这些文献中所展示的关键参数，结合前文中所示的Vaughan模型\ref{eq:SEY_Vaughan}对器件的二次电子发射情况进行仿真。

首先将原器件的背景材料设置为无氧铜，此时无氧铜的导电系数为$114514$，之前的蓝宝石材料电参数已经在
