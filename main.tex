...
此时假设窗片外侧的铜制外壳在与空气进行强制对流散热，设置铜制外壳的空气对流的对流系数为\(300 W/ (m \cdot K)\)，此时外界热对流的设置如图\ref{fig:X频段窗片对流散热}所示，室温被设置为20℃。关于对流散热面的设置窗片表面由于焊接在器件内部，此时假设其在纵向波导内部无热对流，只假设其暴露在真空的单元存在辐射散热，并且取辐射因子为0.22。
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.35\linewidth]{pic/chapter3/}
